...
public PeakList filter(PeakList peaklist) {
		...
		for (int i=0;i<peaks.length;i++){
			...
			acs[i] = new AssignChargePeakListFilterPeak();
...
//		}
	...
	private void assignChargeStates(GenericPeak[] peaks) {
		...
		for (int i = 0; i < peaks.length; i++) {
			...
			if (!foundCharge) {
				peaks[i].setCharge(ChargeAssignedPeak.UNKNOWN_CHARGE);
			}
		}
	}
}
...
/**
 * Custom peak for holding charge information.
 * 
 * @author Jayson Falkner - jfalkner@umich.edu
 */

class AssignChargePeakListFilterPeak extends GenericPeak implements
		ChargeAssignedPeak {
