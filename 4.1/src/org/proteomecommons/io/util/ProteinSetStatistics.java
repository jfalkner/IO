...
import org.proteomecommons.io.fasta.FASTAProteinReader;
import org.proteomecommons.io.fasta.FASTATrypticPeptideReader;
	...
	public void calcStatistics(String fastaFile) {
		try {
			...
			for (Protein p = fpp.next(); p != null; p = fpp.next()) {
				...
				byte[] residues = p.getSequenceAsBytes();
				for (int i = 0; i < residues.length; i++) {
					// increment the total residue count
					singleResidueCounts[(char) residues[i]]++;
					// increment per protein count
					residueCounts[(char) residues[i]]++;
