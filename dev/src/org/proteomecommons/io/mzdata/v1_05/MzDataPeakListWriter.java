...
public class MzDataPeakListWriter extends GenericPeakListWriter {
     ...
     */
    public MzDataPeakListWriter(String filename){
        this(filename,
                "Unnamed Sample",
                "Unnamed Source File",
     ...
     * Writes a CvParam tag to the xml document with the specified paramaters.#
     *
     * @param accession the value of the accession attribute
     * @param name the value of the name attribute
     * @param value the value of the value attribute
     * @throws XMLStreamException
     */
    private void writeCvTag( String accession, String name, String value ) throws XMLStreamException {
        xmlw.writeEmptyElement( "cvParam" );
        xmlw.writeAttribute( "cvLabel", "psi" );
        xmlw.writeAttribute( "accession", accession );
        xmlw.writeAttribute( "name", name );
        xmlw.writeAttribute( "value", value );
    }
     ...
     */
    public void close() {
        // first write out the meta-info
        try {
            writeMetaInfo();
        } catch (XMLStreamException e1) {
            throw new RuntimeException("couldn't write XML metadata.", e1);
        }
        ...
        try{
            ...
            for(int spectrumId = 1; spectrumId <= peaklists.size(); spectrumId++){
                if (peaklists.get(spectrumId -1) != null)
                    encodeAPeakList((PeakList)peaklists.get(spectrumId -1), spectrumId);
            }
            ...
            throw new RuntimeException("Could not close the xml document", xse);
        }
    ...
    private void writeSpectrumDesc(PeakList list) throws XMLStreamException {
        ...
        PeakList genericTandemPeakList = list;
        ...
        if(list instanceof MzDataPeakList){
            ...
            if(mzlist.getSpectrumSettings() != null){
        ...
        if(list instanceof MzDataPeakList){
            ...
            boolean precursorSpectrum = ( genericTandemPeakList.getParentPeak() == null && genericTandemPeakList.getParentPeakList() == null );
            xmlw.writeAttribute( "msLevel", precursorSpectrum ? "1" : "2" );
        }
        ...
        if(list instanceof MzDataPeakList){
            ...
            Peak parent = genericTandemPeakList.getParentPeak();
            
            if( parent != null ) {
                double mass = parent.getMassOverCharge();
                double intensity = parent.getIntensity();
                
                if( mass != Peak.UNKNOWN_MZ ) {
                    xmlw.writeCharacters( "\n\t\t\t\t\t\t\t" );
                    writeCvTag( "PSI:1000040", "m/z", Double.toString( mass ) );
                }
                if( intensity != Peak.UNKNOWN_INTENSITY ) {
                    xmlw.writeCharacters( "\n\t\t\t\t\t\t\t" );
                    writeCvTag( "PSI:1000042", "Intensity", Double.toString( intensity ) );
                }
                if( parent instanceof GenericPeak ) {
                    GenericPeak genericPeak = (GenericPeak)parent;
                    int charge = genericPeak.getCharge();
                    
                    if( charge != Peak.UNKNOWN_CHARGE ) {
                        xmlw.writeCharacters( "\n\t\t\t\t\t\t\t" );
                        writeCvTag( "PSI:1000041", "Charge State", Integer.toString( charge ) );
                    }
                }
            }
            
            xmlw.writeCharacters("\n\t\t\t\t\t\t");
            xmlw.writeEndElement(); // ionSelection
            xmlw.writeCharacters("\n\t\t\t\t\t");
            xmlw.writeEndElement(); //precursor
        }
        
        xmlw.writeCharacters("\n\t\t\t\t");
        xmlw.writeEndElement(); //precursorList
        xmlw.writeCharacters("\n\t\t\t");
