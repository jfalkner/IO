...
import org.proteomecommons.io.dta.SequestDTAPeakListReader;
import org.proteomecommons.io.pkl.MicromassPKLPeakListReader;
...
public abstract class PeakListReader {
	...
	public static PeakListReader newReader(InputStream is, String fileName) {
		try {
			...
			if (fileName.endsWith("dta") || fileName.endsWith("DTA")) {
				return new SequestDTAPeakListReader(is, fileName);
			} else if (fileName.endsWith("pkl") || fileName.endsWith("PKL")) {
				return new MicromassPKLPeakListReader(is, fileName);
			} else if (fileName.endsWith("mgf") || fileName.endsWith("MGF")) {
	...
	public PeakList getPeakList() throws Exception {
		return getPeakList(false, false);
	}
	...
	/**
	 * Returns the parent peak's intensity.
	 * @return
	 */
