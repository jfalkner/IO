...
//		acplf.setTolerance(ppmTolerance);
		...
		ChargeAssignedPeak[] peaks = (ChargeAssignedPeak[])peaklist.getPeaks();
		Arrays.sort(peaks);
		...
		for (int i = 0; i < peaks.length; i++) {
			...
			if (peaks[i].getCharge() == MonoisotopicPeak.UNKNOWN_CHARGE) {
				continue;
			}
		...
		for (int i = 0; i < peaks.length; i++) {
			...
			if (peaks[i].getCharge() == 1 || peaks[i].getIntensity() <= 0
					|| peaks[i].getCharge() == MonoisotopicPeak.UNKNOWN_CHARGE) {
				continue;
			}

			// figure out the singly charged peak
			double singleMZ = peaks[i].getMassOverCharge()
					* peaks[i].getCharge() - (peaks[i].getCharge() - 1)
					* Atom.H.getMassInDaltons();

			// find the singely charge peak
			ChargeAssignedPeak ps = (ChargeAssignedPeak)findPeak(singleMZ, peaks);
			...
			if (!spent) {
				...
				if (extras.length > 0) {
					...
					Arrays.sort(extras);
					// find the singely charge peak
					ps = (ChargeAssignedPeak)findPeak(singleMZ, extras);
					//if there are matches, add to them
					if (ps != null) {
						if (ps.getCharge() == 1) {
			...
			if (!spent) {
				...
				GenericPeak gp = new GenericChargeAssignedPeak();
...
//		// keep the previous paren't charge
//		monoisotopicPeakList.setParentIonCharge(peaklist.getParentIonCharge());
//		// keep the previous parent's mass over charge in daltons
//		monoisotopicPeakList.setParentIonMassOverCharge(peaklist
//				.getParentIonMassOverCharge());
		// set the centroid flag
		monoisotopicPeakList.setAveraged(CentroidedPeak.RESOLVED);
